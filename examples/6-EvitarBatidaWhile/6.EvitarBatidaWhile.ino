...
void setup() {
  ...
  Serial.begin(115200);
  Serial.print("CARRO MODO AUTONÔMO EVITANDO BATIDA MELHORADO");
  Serial.println("");
...
void loop() {
  ...
  if(distanciaCm <= distanciaSegura){ //Se a condição for verdadeira executa as próximas intruções
    ...
    distanciaCm = carEsp32.getDistancia();  //Faz a leitura novamente
    while(distanciaCm <= distanciaSegura*1.5){  //Enquanto a condição for verdadeira executa as intruções dentro do bloco{}
      carEsp32.moverAtras();
      delay(300);
